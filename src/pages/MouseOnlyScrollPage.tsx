...
export default function MouseOnlyScrollPage() {
  ...
  // Handle infinite scroll detection via intersection observer on sentinel or scroll event
  const handleScroll = () => {
    if (!scrollRef.current) return;
    const { scrollTop, scrollHeight, clientHeight } = scrollRef.current;
    
    // Load more when scrolled within 150px of the bottom
  ...
  useEffect(() => {
    const preventKeyboardScroll = (e: KeyboardEvent) => {
      // List of keys that can scroll the page
      const scrollKeys = [
    ...
    // We add to both window and container to ensure it's blocked everywhere on this page
  ...
  return (
    <div className="flex h-screen w-full bg-[#fdf8ff] text-[#1c1b1f] font-sans flex-col p-8 overflow-hidden">
      ...
      <div 
        ...
        className="flex-1 w-full max-w-4xl mx-auto bg-white rounded-xl shadow-md border border-[#d8cfe5] overflow-y-auto outline-none"
